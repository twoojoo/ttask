...
func WithCustomKey[T any](extractor func(x T) string) task.Operator[T, T] {
	return func(inner *task.Inner, x *task.Message[T], step *task.Step) {
...
func Map[T, R any](cb func(x T) R) task.Operator[T, R] {
	return func(inner *task.Inner, x *task.Message[T], next *task.Step) {
		inner.ExecNext(task.ReplaceValue(x, cb(x.Value)), next)
	}
}

// Map the message value (with access to task metadata and message metadata).
// Also allows to create custom operators.
func MapRaw[T, R any](cb func(inner *task.Inner, x *task.Message[T]) R) task.Operator[T, R] {
	return func(inner *task.Inner, x *task.Message[T], next *task.Step) {
		inner.ExecNext(task.ReplaceValue(x, cb(inner, x)), next)
	}
}
...
func FilterRaw[T, R any](cb func(inner *task.Inner, x *task.Message[T]) bool) task.Operator[T, T] {
	return func(inner *task.Inner, x *task.Message[T], next *task.Step) {
		ok := cb(inner, x)
		if ok {
			inner.ExecNext(x, next)
...
func TapRaw[T any](cb func(inner *task.Inner, x *task.Message[T])) task.Operator[T, T] {
	return func(inner *task.Inner, x *task.Message[T], next *task.Step) {
		cb(inner, x)
		inner.ExecNext(x, next)
	}
